...
- 3월 27일 목요일 - 딥러닝 - 모델 검증
    - 분류 모델
        - 교차 검증
            - K_폴드
        - 평가 메트릭스 (혼동행렬)
            - 정확도(Accuracy), 정밀도(Precision), 재현율(Recall), F1-score 등
            ![alt text](이미지/image10.png)
        - 정확도
            - $ \text{Accuracy} = \frac{TP + TN}{TP + TN + FP + FN} $
            - TP: True Positive (실제로 긍정인 것을 긍정으로 예측)
            - TN: True Negative (실제로 부정인 것을 부정으로 예측)
            - FP: False Positive (실제로 부정인 것을 긍정으로 예측)
            - FN: False Negative (실제로 긍정인 것을 부정으로 예측)
        - 정밀도(Precision)
            - 긍정 예측 중 실제 긍정 인것의 정확도
        - 재현율(Recall)
            - 실제 긍정 중 모델이 긍정으로 예측 한 비율
        - F1-Score
            - 정밀도와 재현율의 조화 평균
        - AUC 와 ROC
            - 참긍정 비율과 거짓 긍정 비율의 관계를 나타내는 곡선
            - 참 긍정 비율(TPR)=재현율(Recall), 거짓 긍정 비율(FPR) = 모델이 잘못 양성으로 예측한 비율
            - $ TPR = \frac{TP}{TP + FN}, \quad FPR = \frac{FP}{FP + TN} $
            - 가운데 검은선  0.5 
            - ACU가 0.5를 넘으면 어느 정도 나온다, 1에 가까우면 성능이 좋다 
            - **AUC = 0.5**: 무작위 추측과 동일
            - **AUC > 0.5**: 분류 성능이 무작위 추측보다 나음
            - **AUC = 1**: 완벽한 분류 성능
            ![alt text](이미지/image11.png)
    - 회귀 모델
        - 평균 절대 오차(MAE)
        - 평균 제곱 오차(MSE)
        - 평균 제곱근 오차(RMSE)
