...
with col1:
    ...
    selectes = st.selectbox('학교 입력: ',options)

    r_options = ['맛집 탐방', '영화 감상', '음악 감상', '사진찍기']
    choice = st.radio('취미입력', r_options)
    selecte_many = st.multiselect('취미가 여러 개라면',r_options)
...
with col2:
    if st.button('입력 완료!'):
        ...
        st.write(f'어디 다녀? {selectes}')
        st.write(f'취미가 뭐야? {choice}')
        st.write(f"취미가 여러개야?{selecte_many}")
